...
| Conformance | Published with denominator | **94.6% of 2,557 decided; 98.9% of 2,585 reach a decision** | 🟡 |
...
### 3. Conformance — 138 failures, only 63 of which need the external subset

Every remaining failure is the parser being **too permissive**; there
is no document in the suite it wrongly rejects.

They split by what the failing document contains: **70 internal subset
only, 63 external subset, 5 no `DOCTYPE` at all**. So 75 are rules that
...
needed the external subset; it was wrong. **25 failures have been fixed
since counting began** — 163 to 138 — and the external-subset group has
not moved: 63 before, 63 now.
